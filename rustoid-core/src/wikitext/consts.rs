...
/// The set of HTML5 tag names, used by the tokenizer to distinguish real HTML
/// tags from tag-like text (a `<President>` is *not* an HTML tag and so is
/// emitted as entity-encoded text). Mirrors `Consts::$HTML['HTML5Tags']`.
pub fn html5_tags() -> &'static HashSet<String> {
    static SET: once_cell::sync::Lazy<HashSet<String>> = once_cell::sync::Lazy::new(|| {
        set![
            "a",
            "abbr",
...
/// Obsolete HTML tags still recognized by the tokenizer (mirrors
/// `Consts::$HTML['OlderHTMLTags']`).
pub fn older_html_tags() -> &'static HashSet<String> {
    static SET: once_cell::sync::Lazy<HashSet<String>> =
        once_cell::sync::Lazy::new(|| set!["strike", "big", "center", "font", "tt"]);
    &SET
}
