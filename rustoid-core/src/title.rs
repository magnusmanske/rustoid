...
pub(crate) fn normalize_resolved_subpage(s: &str) -> String {
    s.trim_end_matches('/').to_string()
}

/// Resolve a possibly-relative title reference (`#fragment`, `/subpage`,
/// `../sibling`) against a context title, mirroring PHP's `Env::resolveTitle`.
/// Returns the resolved string (may still carry a leading `:` that the caller's
/// `TitleParser` treats as force-mainspace). Used by both wikilink-target
/// resolution and template-target resolution (`{{/Subpage}}`).
pub(crate) fn resolve_subpage(
    config: &dyn SiteConfig,
    context_title: Option<&Title>,
    str_in: &str,
) -> String {
    let orig_name = str_in.to_string();
    let s = str_in.trim();

    let Some(title) = context_title else {
        return orig_name;
    };
    ...
    if config.namespace_has_subpages(title.namespace_id) {
        ...
        if let Some(rel_up_len) = count_leading_dotdot_slashes(s) {
            ...
            title_key = normalize_resolved_subpage(&title_key);
