...
/// Look up a semicolon-terminated HTML5 named-entity name in
/// `HTMLData::NAMED_ENTITY_TRANSLATION`.
///
/// Only the semicolon-terminated forms are relevant for wikitext: Parsoid's
/// `Sanitizer` entity decoding requires the trailing `;` (see
/// `CHAR_REFS_REGEX`). The legacy no-semicolon forms in the PHP table are
/// intentionally omitted here. Some entities decode to two codepoints.
pub fn named_entity_translation(name: &str) -> Option<&'static str> {
    Some(match name {
        "Aacute;" => "Á",
        "aacute;" => "á",
        "Abreve;" => "Ă",
...
mod tests {
    ...
    #[test]
    fn test_named_entity_translation() {
        assert_eq!(named_entity_translation("amp;"), Some("&"));
        assert_eq!(named_entity_translation("lt;"), Some("<"));
        assert_eq!(named_entity_translation("Aacute;"), Some("Á"));
        assert_eq!(named_entity_translation("acE;"), Some("∾̳"));
        assert_eq!(named_entity_translation("nbsp;"), Some("\u{a0}"));
        assert_eq!(named_entity_translation("notAREAL;entity;"), None);
    }
}
