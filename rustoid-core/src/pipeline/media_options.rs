...
fn prefix_option_info(
    ...
    for (canonical, entry) in magic_words {
        ...
        for alias in &entry.aliases {
            // A parameterized alias is `prefix=$1` (e.g. `link=$1`), `$1suffix`
            // (e.g. `$1px` for `img_width`), or `$1` (capture everything). The
            // literal pieces around `$1` must match, and `$1` captures the rest.
            if let Some(dollar) = alias.find("$1") {
                let literal_prefix = &alias[..dollar];
                let literal_suffix = &alias[dollar + 2..];
                if !opt_text.starts_with(literal_prefix) {
                    continue;
                }
                let after_prefix = &opt_text[literal_prefix.len()..];
            ...
            let literal = alias.trim_end_matches('=');
            if literal.is_empty() {
                continue;
            }
...
/// Parse a media dimension string like `200px`, `200x300`, `x200px` (a trailing
/// `px` or an explicit `x` is required for `img_width`). Mirrors the essential
/// part of `Utils::parseMediaDimensions` for the *already-matched* `img_width`
/// value. Note: the width is resolved via the `$1px` alias (handled in
/// `prefix_option_info`), so a bare number (no `x`/`px`) is *not* a width.
pub fn parse_media_dimension(s: &str) -> Option<String> {
    let s = s.trim();
